...
if(mobile){
	...
	if(localStorage.getItem("code")){
		...
		if(localStorage.getItem("pid")){
			...
			document.getElementById("autojoin-warn").style.bottom = "3vmin";
			setTimeout(() => document.getElementById("autojoin-warn").style.bottom = "", 3000);
		}
...
function startGame(){
	...
	function update(){
		...
		if((alivePlayers > 1 && (selectedMode != 1 || mScore < modeVal)) || (deathFrames -= warp) > 0)
			requestAnimationFrame(update);
		else{
			setTimeout(function(){
		...
		for(var i in players){
			...
			p.data.mov.x *= Math.pow(0.98, warp);
			p.data.mov.y *= Math.pow(0.98, warp);

			p.pos.xv += p.data.mov.x * SPEED * warp;
			p.pos.yv += p.data.mov.y * SPEED * warp;

			p.pos.xv *= Math.pow(0.98, warp);
			p.pos.yv *= Math.pow(0.98, warp);

			p.pos.x += p.pos.xv * warp;
			p.pos.y += p.pos.yv * warp;
		...
		for(var i = 0; i < bullets.length; i++){
			...
			ctx.beginPath();
			ctx.moveTo(b.x, b.y);
			b.x += Math.cos(b.r) * BSPEED * warp;
			b.y += Math.sin(b.r) * BSPEED * warp;
			ctx.lineTo(b.x, b.y);
			ctx.stroke();
		...
		for(var i = 0; i < particles.length; i++){
			var p = particles[i];
			p.x += Math.cos(p.r) * p.s * warp;
			p.y += Math.sin(p.r) * p.s * warp;
			p.r += p.rv * warp;
			...
			p.s *= Math.pow(0.98, warp);
			p.rv *= Math.pow(0.98, warp);
			p.g *= Math.pow(0.98, warp);
		...
		if(!paused){
			...
			replay.push(rdata);
			if(replay.length > Math.round(16 * 6 / warp))
				replay.splice(0, 1);
		}
	...
	function showReplay(){
		...
		for(var i in f.players){
			...
			p.data.mov.x *= Math.pow(0.98, (1 / replaySpeed) * warp);
			p.data.mov.y *= Math.pow(0.98, (1 / replaySpeed) * warp);

			p.pos.xv += (1 / replaySpeed) * p.data.mov.x * SPEED * warp;
			p.pos.yv += (1 / replaySpeed) * p.data.mov.y * SPEED * warp;

			p.pos.xv *= Math.pow(0.98, (1 / replaySpeed) * warp);
			p.pos.yv *= Math.pow(0.98, (1 / replaySpeed) * warp);

			p.pos.x += (1 / replaySpeed) * warp * p.pos.xv;
			p.pos.y += (1 / replaySpeed) * warp * p.pos.yv;
		}
		...
		for(var i = 0; i < f.bullets.length; i++){
			...
			if(frame % replaySpeed == 0)
				continue;
			b.x += (1 / replaySpeed) * Math.cos(b.r) * BSPEED * warp;
			b.y += (1 / replaySpeed) * Math.sin(b.r) * BSPEED * warp;
		}
		...
		for(var i = 0; i < f.asteroids.length; i++){
			...
			if(frame % replaySpeed == 0)
				continue;
			a.x += (1 / replaySpeed) * a.xv * warp;
			a.y += (1 / replaySpeed) * a.yv * warp;
			a.r += (1 / replaySpeed) * a.rv * warp;
			if(a.x < -2 * a.rad)
				a.x += width + 4 * a.rad;
			if(a.y < -2 * a.rad)
				a.y += height + 4 * a.rad;
		...
		for(var i = 0; i < f.particles.length; i++){
			...
			p.x += (1 / replaySpeed) * warp * Math.cos(p.r) * p.s;
			p.y += (1 / replaySpeed) * warp * Math.sin(p.r) * p.s;
			p.r += (1 / replaySpeed) * warp * p.rv;
			p.s *= Math.pow(0.98, (1 / replaySpeed) * warp);
			p.rv *= Math.pow(0.98, (1 / replaySpeed) * warp);
			p.g *= Math.pow(0.98, (1 / replaySpeed) * warp);
		}
...
function startPlayer(){
	...
	function update(){
		...
		ctx.beginPath();
		ctx.arc((isSwapped ? 3 : 1) * width / 4, height / 2, height / 6, 0, 2 * Math.PI);
		ctx.fill();
		ctx.fillStyle = "#222";
		ctx.beginPath();
		ctx.arc((isSwapped ? 1 : 3) * width / 4, height / 2, height / 4, 0, 2 * Math.PI);
		ctx.fill();

		ctx.fillStyle = "hsl(" + me.data.color + ", 100%, 50%)";
		// ctx.fillStyle = "white";
		ctx.font = height / 4 + "px DotGothic16";
		ctx.fillText("A", (isSwapped ? 3 : 1) * width / 4 - height / 16, height / 2 + height / 12);

		ctx.beginPath();
		ctx.arc(
			(isSwapped ? 1 : 3) * width / 4 + height / 4 * buttons.Joystick[0],
...
// 		}
	...
	c.addEventListener("touchstart", function(e){
		...
		for(var i = 0; i < e.changedTouches.length; i++){
			...
			if(Math.hypot(x - (isSwapped ? 1 : 3) * width / 4, y - height / 2) < height * 0.4)
				if(!arrCont("J"))
					touchArr[t.identifier] = "J";
			if(touchArr[t.identifier] == "" && Math.hypot(x - (isSwapped ? 3 : 1) * width / 4, y - height / 2) < height * 0.5)
				if(!arrCont("A")){
	...
	function tMove(e){
		...
		for(var i = 0; i < e.changedTouches.length; i++){
			...
			if(touchArr[t.identifier] == "J"){
				...
				var po = [(x - (isSwapped ? 1 : 3) * width / 4) / height * 4, (y - height / 2) / height * 4];
